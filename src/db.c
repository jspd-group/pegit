...
int create_schema(struct _database_ *db)
{
    ...
    change_context(db);
    printf("CREATE TABLE project( project_id INT NOT NULL AUTO_INCREMENT "
           "PRIMARY KEY, project_name VARCHAR(30) , version_id INT NOT NULL);");
    if (mysql_query(&db->mysql,
                    "CREATE TABLE project( project_id INT NOT NULL "
                      ...
                      int project_id, int version)
{
    ...
    while (listadd != NULL) {
        ...
        if (abc == 0) {
            ...
            int check = 0;
            if (check != 1) {
                add_single_file(db, project_id, listadd->path,
                                listadd->_version);
            }
        }
...
int update_project_version(struct _database_ *db, char *project_name)
{
    ...
    strcat(
        str,
        "UPDATE project SET version_id = version_id+1 WHERE project_name = '");
...
mysql_free_result(result);
*/
int loop_back_one(struct _database_ *db, int project_id) { change_context(db); }